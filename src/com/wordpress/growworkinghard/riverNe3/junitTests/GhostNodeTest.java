...
public class GhostNodeTest {

    @Test
    public void node() {

        Component parent = createNode("", 5);
        Component ghostNode = createNode("GhostNode", 10);
        Component child1 = createNode("", 20);
        Component child2 = createNode("", 21);
        Results result1 = createResults();
        Results result2 = createResults();

        ghostNode.notify(child1.getConnections().getID(), result1);
        ghostNode.notify(child2.getConnections().getID(), result2);

        ghostNode.runSimulation(parent);
    ...
    private Component createNode(final String nodeType, final Integer id) {
        ...
        else if (nodeType.equals("GhostNode")) return new GhostNode(connKeys, layer, basin);
